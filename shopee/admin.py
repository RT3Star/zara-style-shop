...
class CategoryAdmin(admin.ModelAdmin):
    list_display = ('name', 'slug', 'get_image_preview', 'product_count', 'is_active', 'created_at')
...
class ProductAdmin(admin.ModelAdmin):
    list_display = (
    'name', 'get_image_preview', 'price', 'stock', 'category', 'is_active', 'is_bestseller', 'views_count')
    list_filter = ('category', 'is_active', 'is_new', 'is_bestseller', 'gender', 'created_at')
    search_fields = ('name', 'description', 'slug')
    prepopulated_fields = {'slug': ('name',)}
    list_editable = ('price', 'stock', 'is_active', 'is_bestseller')
    readonly_fields = ('get_image_preview', 'views_count', 'average_rating_display', 'created_at', 'updated_at')
    ...
    fieldsets = (
        ...
        ('Зображення', {
            'fields': ('image', 'get_image_preview', 'images')
        }),
        ('Наявність та статус', {
            'fields': ('stock', 'is_active', 'is_new', 'is_bestseller')
        }),
        ('Метрики', {
            'fields': ('views_count', 'average_rating_display')
        }),
        ('Дати', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class ReviewAdmin(admin.ModelAdmin):
    list_display = ('product', 'user', 'rating', 'comment_preview', 'is_approved', 'created_at')
    ...
    fieldsets = (
        ...
        ('Модерація', {
            'fields': ('is_approved', 'admin_reply')
        }),
        ('Дати', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
